...
| **Background Jobs** | `@nestjs/bullmq` + Redis | Distributed memory queue executing inside the monolithic process boundaries. Ideal for resource-heavy operations like bulk CSV parser operations and transactional alerts.                                                                                        |
...
1.  **Database-Per-Tenant**: Highest isolation, separate physical/logical databases.
    - _Downside:_ Expensive, hard to scale database connection pools inside a single monolith running on a modest VPS, and complex dynamic connection-pooling management at runtime.
2.  **Schema-Per-Tenant**: Separate PostgreSQL schemas inside a single database. Good isolation.
    - _Downside:_ Schema migrations become increasingly complex as the tenant base grows (running migrations across N schemas synchronously), leading to high maintenance overhead.
...
To systematically block data leaks without relying on developer discipline, the system combines NestJS middleware, guards, interceptors, and TypeORM:

1.  **Tenant Extraction Middleware (`TenantMiddleware`)** `[COMPLETE]`:
    - Runs early in the request pipeline before guards and interceptors.
    - Extracts tenant identity from the `X-Tenant-Id` header.
    - Validates the extracted identifier as a UUID and queries the database to confirm the tenant exists and is active.
    - Attaches the resolved `tenant` entity and `tenantId` to the request object.
    - Excluded for tenant management routes (CRUD on `/tenants`).
2.  **Tenant Authorization Guard (`TenantGuard`)** `[COMPLETE]`:
    - Acts as a security checkpoint before route handler execution.
    - Checks for the `@Public()` decorator metadata — public routes skip tenant validation.
    - Verifies that `req.tenant` exists and `status === ACTIVE`.
    - If tenant context is missing or tenant is inactive, it throws a `403 Forbidden` exception.
    - Registered globally via `app.useGlobalGuards()`.
3.  **Tenant Context Propagation (`TenantInterceptor`)** `[COMPLETE]`:
    - Wraps request execution in `AsyncLocalStorage` context via `tenantAsyncStorage.run()`.
    - Makes `tenantId` accessible to all downstream services without explicit parameter passing.
4.  **Row-Level Filtering (Service-Layer Enforcement)** `[PARTIAL]`:
    - All services manually inject `tenantId` into query conditions (e.g., `where: { tenantId }`).
    - Each entity includes a `tenant_id` column with a B-Tree index and FK constraint to the `tenants` table.
    - **Planned but not yet implemented:** A `TenantBaseRepository` class that automatically injects `tenantId` constraints into all queries via Custom Query Builders, eliminating the need for manual filtering in every service method.
...
#### 5.1 Authentication & Authorization Module `[NOT STARTED]`
...
#### 5.2 Tenant Management Module `[COMPLETE]`
...
#### 5.4 Inventory & Warehouse Module `[PARTIAL — Entity only, no service/controller]`
...
#### 5.5 Supplier Module `[PARTIAL — Entity only, no service/controller]`
...
#### 5.6 Import/Export Module `[NOT STARTED]`
...
#### 5.7 Media Storage Module `[NOT STARTED]`
...
#### 5.8 Audit Logging Module `[NOT STARTED]`
...
#### 5.9 Notifications Module `[NOT STARTED]`
...
#### 5.10 Search Module `[NOT STARTED]`
...
#### 5.11 Administration Module `[NOT STARTED]`
...
#### 12.2 Docker Compose Configuration `[PARTIAL]`

The deployment environment is orchestrated using a multi-container Docker Compose setup. Currently implemented services:

- `postgres`: PostgreSQL 17 with health checks and persistent volumes. `[COMPLETE]`
- `pgadmin`: Database management UI for development. `[COMPLETE]`

Planned services (not yet configured):

- `app`: The multi-core NestJS monolithic container. `[NOT STARTED]`
- `redis`: Single-instance Redis acting as the Cache Store and BullMQ broker. `[NOT STARTED]`
- `minio`: Local object storage system mimicking AWS S3 for catalog images. `[NOT STARTED]`
- `nginx`: Dynamic reverse-proxy gateway routing traffic and terminating SSL. `[NOT STARTED]`
